...
- **Ask about the report:** the header chat icon opens **Ask BookieBot** in a floating side panel from any screen, with the selected month and Current/Projected context visible. Closing and reopening retains the draft, answer and ongoing request; changing month/view clears them. Source buttons close the panel and open the relevant screen/section. Questions use read-only tools bound to the authenticated owner through the existing AI service, keep no server conversation history, and cannot modify sheets or move money.
...
Optional **Scriptable Home Screen widgets** pair through **Settings → Widgets**. Each person gets separate revocable read-only access to budget remaining, available today, avatar, mode and update time. No native app or Apple developer membership is required. See [widget script, phone setup and tap/refresh behavior](docs/SCRIPTABLE_WIDGETS.md). Tapping opens the HTTPS browser page; it cannot reliably launch the installed Home Screen web app or sign the browser in.
