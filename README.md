...
Optional **Scriptable Home Screen widgets** pair through **Settings → Widgets**. Compact rows show the connection and mode, with **Last checked / Remove** under **⋯**. **Setup & themes** opens a three-step guide with optional Themes/Help and Back controls that preserve pending setup. Small and Medium show the same canonical budget summary with independent **Editorial** / **Two-tone** appearances. Each person gets separate, revocable read-only access; no native build or Apple developer membership is required. Replace the existing Scriptable source without renaming or re-pairing a working script. Website updates do not replace downloaded scripts. A populated widget opens BookieBot in the browser, which may need its own sign-in. See [script, themes, setup and recovery](docs/SCRIPTABLE_WIDGETS.md).
