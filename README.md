...
Optional **Scriptable Home Screen widgets** pair through **Settings → Widgets**. The in-app **Set up widget** guide offers script copying and Back to Settings without losing a pending setup code. Each person gets separate revocable read-only access to budget remaining, available today, avatar, mode and update time; Small and Medium can share the same paired script. Script **v1.3** adds **Editorial** and **Two-tone** themes: run **Theme & preview** for a saved default, or set each widget's Parameter to `editorial` / `two-tone` to display both. Replace the existing source without renaming or re-pairing a working script. **Connected / Last checked** confirms a successful budget read. No native app or Apple developer membership is required. See [widget script, phone setup and pairing recovery](docs/SCRIPTABLE_WIDGETS.md). A populated widget opens the HTTPS browser page; the script opens Scriptable when pairing/retry is needed. Website updates do not replace downloaded scripts. Widget access cannot launch the installed Home Screen web app reliably or sign the browser in.
