...
The phone app adds **Reconcile** when the signed-in person has a watched connected bank account. **Needs review**, **Pending**, and **Checked** separate actionable transactions from authorizations and history. **Check** syncs/rescores existing entries; **Confirm match** links exact recorded amounts without creating or changing a sheet expense. Mismatches stay visible for correction in BookieBot. Bill/subscription aliases, occurrence-scoped matches and guarded review versions prevent missed recurring payments, false matches and duplicate confirmations. See [reconciliation behavior and verification](docs/RECONCILIATION.md).
...
- **Screens:** the Konsta navigation bar opens **Overview** (four headline metrics, comparisons and all four graphs), **Spending** (daily spending and largest/frequent expenses), **Shared**, and **Savings**, with **Reconcile** added for watched connected bank accounts. While scrolling down it contracts into a centered, narrow icon-only capsule; scrolling up restores the full bar and labels. Every control retains a full touch target. Switching tabs preserves each screen's scroll position and open controls/forms during the current app session. **Settings**, opened with the header gear, contains appearance, phone notifications, account/signout and app updates. Overview and Spending share the selected month and Current/Projected view; canonical reimbursements, savings and recent bank review use their own time scopes.
...
- **Edit split expenses in Discord:** recent transactions expose Update, Move, Split and Delete. Update edits purchase details and recalculates shares when the gross amount changes; Split edits the allocation and includes confirmed Cancel split. Resolve pending reports and reverse confirmed repayments before correcting the purchase. Changes retain audit history and recover safely if Sheets is still syncing; [acceptance and recovery steps](docs/REIMBURSEMENTS.md#editing-a-split-in-discord) cover the workflow.
