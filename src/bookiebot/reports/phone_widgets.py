...
<h2>Seeing “Pair this phone”?</h2><p>Replace the code in your existing Scriptable BookieBot script with version 1.2 from Settings → Widgets → Set up widget; keep its name. Earlier versions could lose authorization after pairing. Run it inside the Scriptable app and paste a fresh setup code only if prompted. Confirm your name and figures, then run it again to check the connection survived. Refresh Widgets in BookieBot: Connected / Last checked confirms a successful read. Home Screen previews cannot show the pairing prompt.</p>
<p>Small and Medium can both select that same paired script in Edit Widget; leave Parameter empty and When Interacting at Open App. The script sets its tap destination. If you renamed or imported another copy, choose the exact script you paired.</p>
