...
<style>body{{font:17px/1.6 system-ui;background:#17201c;color:#edeae2;max-width:640px;margin:40px auto;padding:0 24px}}h1{{font:34px Georgia}}a{{color:#9bc9b5}}li{{margin:16px 0}}code{{overflow-wrap:anywhere}}nav a{{display:inline-flex;align-items:center;min-height:44px}}nav:last-child{{margin:32px 0}}</style>
...
<li>Install <a href="https://apps.apple.com/app/scriptable/id1405459188" target="_blank" rel="noreferrer">Scriptable</a> on your iPhone. No Apple developer account is needed.</li>
<li>In <a href="/app/expenses#settings">Settings → Widgets → Set up widget</a>, tap <strong>Copy script</strong>. In Scriptable, tap +, paste the code and name the script BookieBot. If copying is blocked, select and copy the code shown in the setup guide. This keeps setup inside BookieBot with Back controls instead of opening a raw download.</li>
...
<li>Open the Scriptable app, open BookieBot and tap its ▶ run button. Paste the copied setup code when asked. Opening the setup link in a browser only shows this guide; it does not pair the widget. Allow its BookieBot network requests. Confirm your name and figures in the preview.</li>
<li>Long-press your Home Screen, choose <strong>Edit → Add Widget → Scriptable</strong>, and add a small or medium widget. Long-press it → Edit Widget → Script → BookieBot. Leave Parameter blank to use your saved theme. The script sets the tap URL.</li>
...
<h2>Choose a theme</h2><p>Already connected? Replace the code in the same Scriptable script with version 1.3 from Settings → Widgets → Set up widget; keep its name. No new pairing is needed. Run the script → <strong>Theme &amp; preview</strong> → <strong>Editorial</strong> or <strong>Two-tone</strong> → Small or Medium. Editorial is the starting default.</p>
<p>To show both designs, select that same script for each widget and set <strong>Edit Widget → Parameter</strong> to <code>editorial</code> or <code>two-tone</code>. An empty Parameter uses your saved theme. Never paste a setup code into Parameter.</p>
<h2>Seeing “Pair this phone”?</h2><p>Install the latest script, version 1.3, in the same Scriptable BookieBot script without renaming it. It retains version 1.2's fix for lost authorization after pairing. Run it inside the Scriptable app and paste a fresh setup code only if prompted. Confirm your name and figures, then run it again to check the connection survived. Refresh Widgets in BookieBot: Connected / Last checked confirms a successful read. Home Screen previews cannot show the pairing prompt.</p>
<p>Small and Medium can both select that same paired script in Edit Widget; leave When Interacting at Open App. The script sets its tap destination. If you renamed or imported another copy, choose the exact script you paired.</p>
...
<p>iOS chooses refresh timing; 15 minutes is a request, not a guarantee. The server can reuse a snapshot for up to five minutes. The last-updated date and time identify the source snapshot. On a failed refresh, older figures are labeled stale; never assume it is live. Unknown values show a dash.</p>
<p>A widget showing figures opens <a href="{origin}/app/expenses">BookieBot in the browser</a> when tapped. A widget asking for setup, reconnection or its first data opens its script in Scriptable instead. Browser links cannot reliably open your installed Home Screen app. The browser may need its own /expense_app sign-in; widget pairing never signs it in. If this guide opened outside your app, you can also return using the BookieBot Home Screen icon.</p>
...
<p>Connections expire after one year. Web sign-out leaves separately paired widgets active; /expense_app_reset revokes all phone and widget connections for that account. To clear this phone’s local copy immediately, run the script and choose Forget this phone, then remove the connection in BookieBot. For script updates, replace the contents of your existing BookieBot script and keep its name; importing a duplicate creates a different pairing profile.</p>
