...
Verify Brian/Hannah identity and default scope, cold/foreground/manual refresh, remembered mode/chart, history selection and **This month**, matching-day comparisons, offline/stale status, and explicit update/Later behavior. Check chart/day details and all four metric explanations. Ask about each mode and confirm answers clear when changing it. Create/contribute/reverse/archive/restore a test goal and confirm monthly Saved and the other account are unchanged.
