...
Follow [the Scriptable setup and pairing recovery guide](SCRIPTABLE_WIDGETS.md). If pairing disappears, install script **v1.2** by replacing the existing script's contents without renaming it; earlier versions could lose authorization on their first read. Run it inside Scriptable and pair with one fresh setup code only if prompted. Verify amounts, run it again, then refresh Settings to check **Connected / Last checked**. Small and Medium can share that same script. iOS controls refresh timing, so inspect the timestamp/age and stale label. Tapping a widget with figures opens the browser's BookieBot page, which may need its own sign-in; the revised script opens Scriptable when pairing/retry is needed. Use your existing BookieBot icon to open the installed web app directly. No native build or Apple developer membership is needed.
