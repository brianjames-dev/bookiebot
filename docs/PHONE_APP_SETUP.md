...
If pairing disappears, the current script retains v1.2's fix for lost authorization on the first read. Pair with one fresh setup code only if prompted. Verify amounts, run it again, then refresh Settings to check **Connected / Last checked**. Small and Medium can share that same script. iOS controls refresh timing, so inspect the source timestamp and stale label. Tapping a widget with figures opens the browser's BookieBot page, which may need its own sign-in; the script opens Scriptable when pairing/retry is needed. Use your existing BookieBot icon to open the installed web app directly. No native build or Apple developer membership is needed.
