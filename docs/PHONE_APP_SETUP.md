...
The Konsta navigation bar becomes compact while you scroll down and expands when you scroll up. Switching screens keeps their scroll positions, open controls and forms during the current app session. A compact glass toolbar keeps chat and Settings visible at the top right as you scroll. The gear opens **Settings**; **Back** returns to the last main screen. The chat icon opens Ask BookieBot from any screen. Signed report links retain their existing report layout; this navigation belongs to the authenticated phone app.
...
Open **Settings → Widgets → Setup & themes** to copy the Scriptable script, then return and pair this person's widget. The guide has three short steps and closed **Themes** / **Help** disclosures. **Back to Settings** preserves any in-progress setup code. Public help pages also have a Settings return link. It shows budget remaining, available today, the BookieBot avatar, your name, Current/Projected mode and its source update time. Brian and Hannah create separate connections. The mode selector stays in each compact row; **⋯** holds **Last checked** and **Remove**, with confirmation before revoking access; access is read-only and cannot change expenses or sign into the web app.

Follow [the Scriptable setup and pairing recovery guide](SCRIPTABLE_WIDGETS.md). Install script **v1.3** by replacing the existing script's contents without renaming it; a working pairing stays connected. Run it inside Scriptable → **Theme & preview** to choose **Editorial** or **Two-tone**, then Small or Medium. To show both designs at once, select the same script for both widgets and set **Edit Widget → Parameter** to `editorial` or `two-tone`. An empty Parameter uses the saved theme; never paste a setup code there.

If pairing disappears, the current script retains v1.2's fix for lost authorization on the first read. Pair with one fresh setup code only if prompted. Verify amounts, run it again, then refresh the Widgets list to check **Connected**; the connection’s **⋯** menu shows **Last checked**. Small and Medium can share that same script. iOS controls refresh timing, so inspect the source timestamp and stale label. Tapping a widget with figures opens the browser's BookieBot page, which may need its own sign-in; the script opens Scriptable when pairing/retry is needed. Use your existing BookieBot icon to open the installed web app directly. No native build or Apple developer membership is needed.
