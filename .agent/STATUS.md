...
- Kept the carousel shell and every slide flush with the viewport edges while adding a separate responsive `16px`-to-`56px` inner content gutter. The shared shaded surface still enters and exits from the screen edge, but chart labels, plots, and controls no longer touch it. Category Mix, Burn Rate, Calendar, and Bills & Utilities use the same header/body/footer grid, with the graph occupying whatever middle space remains and the lower control region aligned across pages.
...
- Verification: report suite `52 passed`; full suite `509 passed, 1 skipped`; Pyright `0 errors` with the project Python 3.12 environment; frontend typecheck/build passed; desktop and `390x844` browser checks confirmed exact viewport-width slides at `x=0` (`1425px`/`375px` client widths), inset content at `x=56`/`x=16`, no horizontal overflow, fixed `640px`/`540px` stages, aligned lower regions, cleaned headings, complete 10-row subscription details, contained modal scrolling with page-position restoration, and the donor/recipient overspend figures above; browser diagnostics and `git diff --check` were clean.
