...
Student-loan integration (2026-09-09): implemented opt-in expected bill amounts, canonical $59 projection behavior and guarded owner-scoped payment/status commands. Audited Brian/shared September sheets in Chrome; restored only Template D14's status formula. September's $119.92 actual and existing day 12 are preserved pending user confirmation. **Activation is pending explicit approval** to append J `expected_amount` / J3=59 to the populated bill schedule after automatic approval review rejected the structural insertion. No bypass or live payment was attempted. Audit findings, separate shared-total/Template issues and annual-master follow-up are in `docs/STUDENT_LOAN_INTEGRATION.md`; checklist 119 records acceptance. Final local verification: **1,698 passed / 213 optional PostgreSQL skipped**, clean Pyright/frontend typecheck, Apps Script rollover and diff checks.

Approved widget collection implemented (2026-09-09): Upcoming payments, Savings goal, Category budgets and Shared balance in Small/Medium and both existing themes. Script v1.4 uses the centered larger Medium savings ring and balanced Shared columns. Existing read-only pairing serves bounded owner-scoped summaries; blank/theme-only parameters preserve Budget. Setup & widgets → Customize supplies each instance's type/theme/goal parameter. Final verification/release results are recorded below; native phone acceptance is checklist 118.

Widget settings simplification implemented (2026-09-09): compact connection rows, one Setup & themes entry point, three short setup steps and closed Themes/Help disclosures. Distinct lighter surfaces separate all five Settings sections. Preserves pairing/Back/removal and fixes Safari ancestor-focus swallowing menu actions after guide return. Full local suite **1,524 passed / 213 optional PostgreSQL skipped**; focused controls/lifecycle **6 passed**, Pyright/typecheck/build clean. Production-build WebKit passed 320/390/1280px in both themes, including setup-return menu removal, copy/Back, persistence and final surface alignment. At 390px the two-connection section is 256px versus 520px before (51% shorter). Phone acceptance is checklist 117; deployment evidence is recorded in the completion response. The approved collection is now implemented in v1.4 below.

Widget themes implemented (2026-09-09): user confirmed v1.2 pairing works and approved Editorial (A) and Two-tone (C). Script v1.3 supplies both Small/Medium presentations, a Theme & preview chooser and per-instance editorial/two-tone overrides, sharing the same secure profile. Keeps canonical amounts, owner/mode, negative/stale/recovery states and a compact absolute source timestamp. Full local suite: **1,524 passed / 213 optional PostgreSQL skipped**; focused widget suite **79 passed / 36 skipped**; Pyright/typecheck/build clean. Executed-renderer browser approximation covered 72 size/theme/value scenarios; production-build WebKit verified setup/copy/Back at 320/1280px. Native phone theme acceptance is checklist 116; deployment evidence is recorded in the completion response.

Widget authorization recovery (2026-09-09): Scriptable v1.2 assigns complete native header dictionaries and verifies Keychain readback. The previous script failed the new actual HTTP pairing/data contract with a 401 and cleared its credential; v1.2 retains it for Brian/Hannah in both modes. Settings reserves Connected for a successful read, with Paired/first-preview guidance otherwise. Recovery docs explain replacing the existing source, verifying repeat runs and sharing one script across Small/Medium. Local verification: **1,523 passed / 213 optional PostgreSQL skipped**, focused widget suite **78 passed / 36 skipped**, Pyright/typecheck/build clean. Production-build WebKit passed 320/390/1280px in both themes, including Paired → Connected status, copy/fallback, Back and preserved setup state. Phone acceptance is checklist 115; deployment verification is recorded in the completion response.

Widget setup recovery and Settings return navigation completed (2026-09-09): in-app setup replaces raw-file navigation with Copy script and a selectable fallback, with Back controls at both ends. Pending codes, edited forms and Settings scroll survive help/gear/tab detours; public help/connect pages return through the exact #settings route. Scriptable v1.1 uses its own run URL for pairing/retry, confirms the paired identity and stops cleanly on canceled/failed setup. Existing scripts can still be paired directly; adopting v1.1 requires replacing the existing script contents without renaming. Full local suite: **1,519 passed / 213 optional PostgreSQL skipped**; focused final guide/routes/navigation: 32 passed; Pyright/typecheck/build clean. Final production-build WebKit passed 320/390/1280px in both themes, including copy/denial/retry, top/bottom Back, preserved codes/scroll and safe public returns. Phone acceptance is checklist 114.

Ask panel motion and persistent header tools completed (2026-09-09): fixed WebKit native autofocus scrolling the offscreen panel and snapping back; symmetric 240ms motion now finishes before dismissal/source navigation, with safe rapid reversals. A compact Konsta glass Toolbar keeps chat/Settings at the top right while scrolling; taps leave no focus ring and keyboard focus stays visible. Drafts, requests and modal containment remain intact. Full local verification: **1,516 passed / 213 optional PostgreSQL skipped**, clean Pyright/typecheck/build; production-build WebKit frame/lifecycle checks passed at 320/390/1280px in both themes and reduced motion. Phone acceptance is checklist 113.

Scriptable widgets implemented (2026-09-09), after verified navigation release `a408fec`: Settings → Widgets issues separate owner-scoped, revocable read-only connections; small/medium Scriptable widgets show canonical current-month budget remaining/available today, avatar, person, mode and source timestamp/age. Existing web app remains intact. Full verification: **1,729 passed** including real PostgreSQL, clean Pyright/frontend checks and production-build WebKit navigation/lifecycle checks. Setup, cache/refresh limitations, revocation and browser tap behavior are in `docs/SCRIPTABLE_WIDGETS.md`; actual iPhone acceptance is checklist 112. Release fingerprint: `b293849fb2f8220045ce9c7d`.

App navigation implementation completed (2026-09-09): Overview, Spending, Shared and Savings tabs; header Settings; floating Ask BookieBot; month popovers and guarded Update/× toast. Existing financial behavior and refresh remain intact. Financial controllers survive month-loading/error states, screen switches and report replacement; expiry/signout clears the shell. Full verification and device acceptance are recorded below (checklist 111). Implementation commit `70be82f` passed GitHub Verification [34327707408](https://github.com/brianjames-dev/bookiebot/actions/runs/34327707408), including real database contracts and generated-asset parity.

Reimbursement presentation cleanup (2026-09-08): zero outstanding in the selected direction shows a plain message and hides the opposite net summary; an empty direction has no accordion, while real settled history and recovery notices remain accessible. View expenses, Compare spending and comparison Details share the existing animated +/− stroke. Eleven focused UI/motion tests, typecheck and build passed. WebKit passed 46 layout checks and 32 animation cycles at 320/390/1280px, both themes and reduced motion. Empty-state/motion commit `2ff9f9b` is on main, passed CI `34314022723`, and its live frontend fingerprint matches `8215b675314fb35ba0af798d`. The user selected preview A: the direction selector now uses borderless columns, fitted balances and a short sage underline with the existing sliding motion. Focused lifecycle verification, typecheck and build passed. Editorial WebKit acceptance passed 72 layout/style checks and eight bidirectional underline cycles across 320/390/1280px, both themes and reduced motion, including long amounts and keyboard/touch use. Checklist 110 covers the implemented fixes.

Confirmed-repayment reimbursement ledger (2026-09-08): implemented the user's gross-until-repaid model, trusted payer parsing, both debt directions, partial payments, pending sender reports with recipient confirmation, reviewed offsets and whole-group reversals. Canonical database transactions and replayable named-range projections protect balances, source rows and the worksheet mirror. Historical received records remain read-only. Full local verification passed: 1,617 tests including real PostgreSQL, clean Pyright, frontend typecheck/build and Apps Script checks; WebKit covered 48 viewport/theme states and synthetic payment lifecycles. Main implementation `3e7b92f` passed GitHub Verification (`34311282247`) and its production frontend fingerprint matches `e31a6482ae75883e4153aefd`. The reviewed migration registered seven allocations with all projections verified; an independent readback checked all five cash-model sources, both owners' balances, the worksheet mirror and preserved void/received history. Railway feature activation and final live UI evidence are recorded in the completion response; see checklist 109 and `docs/REIMBURSEMENTS.md`.
...
Approved compact Savings goals implemented (2026-09-08): initially collapsed goal rows have left chevrons, exact fitted balances, thin progress bars and separate ⋯ menus. Overview shows only date/progress/target/remaining; starting balance and contributions stay in History. One goal view opens at a time, while existing confirmed actions and uncertainty recovery remain. Focused 41 passed / 27 optional PostgreSQL skipped; full 1,116 passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build clean. WebKit passed six viewport/theme layouts, eighteen edge layouts, keyboard/rapid disclosures and all synthetic goal actions, with no overflow/errors. Phone acceptance is checklist 106; deployment evidence is in the completion response.

Approved reimbursement visual A implemented (2026-09-08): each expanded row uses a slim allocation bar with ivory payer/gross text, exact own/received/owed proportions, and two aligned Yours/partner-paid keys. The due marker appears only beside the expanded row’s due label; collapsed rows remain compact. Date/split/location/fronted and differing budget details remain explicit, with five labeled values for malformed legacy allocations. Focused 15 and full 1,116 tests passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build clean. WebKit verified six viewport/theme layouts, 22 edge states and 36 additional responsive edge cases; exact overview pixels and symmetric 240ms accordion motion are preserved. Phone acceptance is checklist 105; deployment evidence is in the completion response.

Reimbursement compact receipts/accordion implemented (2026-09-08): metadata is consolidated above aligned label/value rows, 18px serif month headings restore the current-month pill, and each month permits one open expense at a time. Distinct payer/budget responsibility remains explicit; fronted copy is accurate. Focused reimbursement/motion 76 and full 1,116 tests passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build clean. WebKit passed six viewport/theme layouts and sixteen edge states with aligned columns, symmetric 240ms accordion motion, rapid/keyboard switching and independent month/Totals states. Overview pixels are unchanged. Phone acceptance is checklist 104; deployment evidence is in the completion response.

Reimbursement receipt styling implemented (2026-09-08): month groups use serif headings and a thin rule; an explicit Totals button/rotating chevron replaces the misleading info icon. Each expense title appears once and unwraps when expanded. Receipt metadata, gross, shares, received amount and split responsibility have distinct layouts. Focused 69 and full 1,116 tests passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build clean. WebKit verified six viewport/theme layouts and fourteen edge states with no overflow/errors; overview pixels are identical to the prior published build. The overview and financial behavior are unchanged. Phone acceptance is checklist 103; deployment evidence is in the completion response.

Reimbursement visual refinement implemented (2026-09-08): one segmented bar shows each expense month’s share of the outstanding balance, with matching legend swatches. Distinct month headings contain an optional totals info disclosure; one-line 44px expense rows retain dates/partners/audit fields on expansion. Focused 69 passed; full 1,116 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. WebKit verified six viewport/theme combinations, thirteen edge states and two exact $118.89 samples, including 320px totals, correct proportions/colors and symmetric 240ms motion, with no overflow/errors. Financial scopes and history recovery remain unchanged. Phone acceptance is checklist 102; deployment evidence is in the completion response.

Refresh/history timeout recovery implemented (2026-09-08): production logs confirmed repeated 20-second Google Sheets ReadTimeout failures, including unrelated reminder reads. Same-container metadata probes later completed in 0.17–0.43s; live Retry and refresh recovered, confirming a transient source timeout plus a sticky catalog warning. Typed source/client-deadline feedback now preserves the last report and timestamp. Each successful report can recheck incomplete/failed history once, after any in-flight catalog read; no failure-driven loop or deadline increase. Focused backend 77 and frontend 3 passed; full 1,116 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. Actual FreshExpenseApp WebKit QA at 320/390px verified deferred recovery, no retry loops, preserved Projected/metrics/timestamp, explicit Retry, comparison recovery and no overflow/errors. Phone acceptance is checklist 101; deployment evidence is in the completion response.

Unified reimbursement list implemented: one month-grouped disclosure combines outstanding and received expenses, with compact rows and expandable split details. Removed redundant captions; dollar bars now have direct amount-due labels and no progress tracks. Canonical received history keeps older carried expenses marked Received after settlement without extra sheet reads. Selected-month totals, open-only chart scope and financial writes remain unchanged. Focused backend/report 97 passed and UI regressions passed; full 1,099 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. WebKit verified six viewport/theme layouts and twelve edge states with no overflow/errors and symmetric 240ms motion. Phone acceptance is checklist 100; deployment evidence is in the completion response.

Shared reimbursement overview redesigned around one outstanding balance and proportional expense-month bars from the same unpaid allocations. Four graph rows maximum, with earlier months combined and undated amounts explicit; full partner/item details and selected-month statement/history start collapsed. Existing chart-band background and symmetric 240ms disclosures remain. Coverage warnings, legacy month scope, partial receipts, original dates and all split fields are preserved. Focused 66 passed; full local 1,096 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. WebKit verified 320/390/1280px in both themes and eight edge states without overflow or page errors. Deployment evidence is in the completion response; phone acceptance is checklist 99.
...
Notification save request fix implemented: same-origin notification and reconnect/signout fetches explicitly retain the app origin under the page no-referrer policy. Server origin checks, cookies and delivery opt-in remain unchanged. Live anonymous probes matched Origin:null to the screenshot 403; valid origin reaches authentication. Full local suite: 1,095 passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build passed. Actual WebKit 26.5 reproduced old null-Origin 403 and passed fixed On/save/reload/off/re-enable with the retained endpoint; Chromium 148 passed the fixed flow too. CI/live evidence is recorded in the completion response.

Repeated-month comparison quota fix implemented. Production logs confirmed Sheets Read requests per minute per user exhaustion. Fresh reports now batch workbook tabs; comparison-year inputs/catalogs reuse bounded process memory for at most 60s and invalidate across years on direct refresh. Real gspread HTTP regression: refresh + eight comparisons cost 10 reads instead of 172; refreshing edited sources and repeating all eight costs 20 total and shows updated values. Full local suite: 1,093 passed / 60 optional PostgreSQL skipped; Pyright and frontend typecheck/build passed. CI/live evidence is reported in the completion response.

Refresh/comparison recovery implemented: comparisons wait for an active main refresh before starting new reads; failed comparisons retry once when refresh finishes; a bounded abort promise releases stalled browser requests. A 10-second comparison-only handoff avoids rebuilding a just-refreshed month, while every regular refresh remains fresh and invalidates old handoffs. Sheets sockets now have 5s connect / 20s read timeouts. Local suite 1,051 passed / 60 optional PostgreSQL skipped; Pyright and frontend typecheck/build passed. Synthetic phone QA reproduced both error banners and recovered through Try again and refresh without reopening or losing Projected. CI/live evidence is in the completion response.

Dashboard reliability/polish pass implemented and verified locally: configurable month comparisons, native iPhone enable/persistent notification preferences, stable dialog geometry/focus, complete compact Burn Rate metrics, pinned mobile category filters, unnumbered chart tabs, no Daily Spending footer rule, and owner/month/refresh alignment. Full suite 1,104 passed including real PostgreSQL; Pyright, frontend typecheck/build, Apps Script and browser checks passed. CI and live deployment evidence is reported in the task completion response.

Current/Projected blank-page fix completed: pie sectors now resolve by stable category identity during chart synchronization, and disappearing sectors cannot open unrelated rows. Full local suite: 1,022 passed / 57 optional PostgreSQL cases skipped; Pyright and frontend typecheck/build passed. Real React/Recharts regression reproduces the prior crash and covers animated/unanimated, shrinking/reordered/empty datasets, rapid swaps and keyboard/click targets. Phone-width browser stress passed 30 mode changes and 12 category-filter changes; live deployment evidence is in the task completion response.
...
Student-loan activation first: obtain approval for the exact J-column addition above and confirmation of September's $119.92 / day 12, then apply and read back the approved configuration. Shared grand totals omit Needs; internal Template imports and separate annual masters need the documented follow-up. Code is backward compatible while configuration remains absent.

1. Optional device acceptance for the completed dashboard: each phone can enable its own notifications, create its own savings goals and pair its own Scriptable widget using `docs/PHONE_APP_SETUP.md` and `docs/SCRIPTABLE_WIDGETS.md`; checklists 91, 93, 94, 95, 96, 97, 98, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117 and 118 cover these explicit personal choices and the latest reliability fixes. Implementation is complete.
...
- 2026-09-09 — Implemented explicit recurring bill expectations and standalone student-loan commands with unique source validation, actual/forecast separation, canonical report/widget parity, stable named payment targets and exact-label recent-action guards. Active matching subscriptions block standalone commands and duplicate estimates/reminders; legacy subscription-only loans remain untouched. Chrome audit verified September totals/links/anchors and repaired the missing Template loan-status formula. Full local **1,698 passed / 213 optional PostgreSQL skipped**, Pyright/frontend typecheck and Apps Script checks passed. README, decisions, Finance Ops work log and `docs/STUDENT_LOAN_INTEGRATION.md` record behavior, source findings, pending J-column approval and manual checklist 119. No payment, shared-total edit or notification was created.

- 2026-09-09 — Implemented four approved widget types in Small/Medium and both themes. v1.4 keeps existing pairing and Budget behavior, adds a separate remembered preview choice, owner-scoped summary routes, per-type/goal offline caches and strict unavailable/stale handling. Category/upcoming use canonical report data; goal/shared reads use their canonical stores without sheet projection writes. Settings Customize generates parameters with selected active goals, copy fallback and preserved Back/pairing state. WebKit verified 320/390/1280px in both themes; fixed native select height. Executed Scriptable renderer checks cover both sizes, themes and edge states, including centered Medium ring/columns and full timestamp width. README, setup guide and decisions document reuse, semantics, security and phone checklist 118. No schema migration or Finance Ops backlog changes.

- 2026-09-09 — Simplified Widgets settings into compact connection rows with direct mode selectors and secondary timestamp/removal controls in ⋯ menus; one Setup & themes guide replaces duplicate entries. Three short setup steps retain Copy, pairing and both Back controls; Themes/Help start closed. Added separate lighter Settings surfaces without decorative dividers, preserved inner mobile widths and clamped long connection names with full labels in options. Shared menu regression protects Safari ancestor focus after returning from setup; outside taps, sibling focus and Escape retain dismissal. Full local **1,524 passed / 213 skipped**, focused **6 passed**, clean Pyright/typecheck/build. Final WebKit functional and geometry checks passed 320/390/1280px in both themes: centered surfaces with 20px spacing, menus after setup return, long names, mode/removal persistence and no overflow/errors. The two-row section is 51% shorter at 390px. Updated in-app/public guides, reference docs, decisions and manual checklist 117; installed Scriptable v1.3, credentials and financial behavior are unchanged. No Finance Ops backlog changes.

- 2026-09-09 — Implemented approved Editorial and Two-tone widget themes in v1.3. Native stacks/gradients and serif/rounded figures adapt to Small/Medium, with negative contrast, full cents and one source timestamp. Script-local theme defaults live separately from secure pairing; explicit per-widget theme parameters allow both designs using one credential, while cancel/error handling leaves pairing unchanged. Added executed theme/render contracts, including scopes, invalid parameters, menu/preview choice and stale/negative/missing data. Full local **1,524 passed / 213 skipped**; focused **79 passed / 36 skipped**; clean Pyright/frontend checks. Browser approximation checked 72 executed-renderer scenarios, full-height sage panels and shrinking amounts; native WidgetKit remains checklist 116. WebKit setup/copy/Back passed at 320/1280px. Updated README, setup guides, decisions and manual acceptance; no financial/schema/Finance Ops backlog changes.

- 2026-09-09 — Fixed widget authorization loss after successful pairing. Native dictionary-copy regression reproduces v1.1 losing request headers; real local HTTP regression reproduces its first-read 401 and credential removal. Script v1.2 assigns complete headers, verifies secure Keychain storage before success and preserves existing valid profiles. Real-route tests cover Brian/Hannah × Current/Projected, subsequent execution, canonical snapshot values and secret-free caches. Settings now distinguishes Paired from Connected/Last checked; setup and phone guides explain replacing source, verifying repeated previews and sharing one script across sizes. Focused widget tests **78 passed / 36 optional PostgreSQL skipped**; full local suite **1,523 passed / 213 skipped**; Pyright/frontend typecheck/build clean. WebKit passed six viewport/theme combinations with copy/fallback, both Back controls, preserved state and accurate readiness labels. Updated README, setup docs, decisions and checklist 115; no financial changes or Finance Ops backlog changes. Release evidence belongs in the completion response.

- 2026-09-09 — Corrected widget setup dead ends: unpaired/revoked/no-data widgets run their exact Scriptable script, pairing confirms identity/script name, and canceled/failed attempts stop without a misleading preview. In-app setup offers Copy script/selectable fallback with Back controls, keeps Settings state mounted, and returns below the fixed toolbar. Public help/connect pages return safely to Settings; raw script endpoint stays available for copying but is no longer a navigation link. Added executed script/guide/shell and public-route regressions; full suite 1,519 passed / 213 optional PostgreSQL skipped, focused final 32 passed, clean Python/frontend checks. Final WebKit passed six viewport/theme configurations without overflow/errors, including clipboard denial, source retry, both Back controls and safe public return links. Updated README, phone/widget guides, decisions and checklist 114. No financial/persistence changes or Finance Ops backlog changes.

- 2026-09-09 — Polished Ask panel and persistent header tools. Measured WebKit's horizontal dialog scroll/snap and fixed stationary autofocus plus overflow clipping; removed redundant moving blur, tied modal dismissal to the actual transform completion, preserved rapid reversal and deferred source navigation until scroll restoration. Added a fixed Konsta Toolbar/ToolbarPane with 44px chat/Settings targets and pointer-versus-keyboard focus styling. Focused lifecycle/navigation regressions: 3 passed. Full local suite: 1,516 passed / 213 optional PostgreSQL skipped; Pyright/typecheck/build clean. Actual WebKit frame QA passed seven viewport/theme/reduced-motion combinations, pending answers, source routing, rapid reversals and scrolled tools on every screen, without console errors. Updated README, phone guide, decisions and checklist 113; no finance/backlog changes.

- Added Settings → Widgets with explicit one-time pairing, memory-only setup codes, copy fallback, per-connection mode/removal and pending-operation guards. Hash-only durable widget credentials are distinct from phone sessions, owner-scoped, short-lived for pairing and revocable/expiring for reads. Concurrent redemption/revocation, lost responses, mapping changes, expiry and resets fail closed; ordinary sign-out preserves separate widget access.
- Added a bearer-only minimal snapshot API reusing canonical mode views and ISO source timestamps. Widget reads share the existing bounded report worker and retain only five minutes of minimal source data; delayed/disconnected requests remain coalesced and timestamps never advance on cache hits or failures. No new expense/budget formulas or financial mutations. The script binds to HTTPS origin, blocks redirects, uses Keychain/local cache, clears revoked credentials, preserves explicit stale timestamps, and sets a credential-free browser tap URL.
- Verification: **1,729 tests passed**, including 72 SQLite/PostgreSQL store cases, executed HTTP auth/scope/canonical calculations and independent cancellation/revocation races, Settings lifecycle and the actual distributed script's API harness. Pyright **0 errors/warnings**, frontend typecheck/build and Apps Script checks passed. WebKit checked Widgets at 320/390/1280px in both themes; the production build additionally passed all screens/charts, mode/theme, scroll restoration, failed refresh/comparison recovery and retained financial drafts/retry identity. Approximate script-tree layouts cover small/medium, stale/unavailable and long names/large signed amounts; actual native iPhone rendering/import/tap/scheduling remain manual. README, setup guide and durable decisions updated. No live personal pairings or synthetic financial records were created. Release/deployment evidence follows in the completion response.

- Implemented the approved four-screen shell using minimal Konsta iOS components and Lucide icons. Overview retains metrics/comparison/four charts; Spending holds Daily Spending and merchant highlights; Shared and Savings retain their complete existing workflows. Settings contains theme, per-phone notifications, account and updates. Navigation compacts on downward scrolling, restores on upward scrolling, and preserves independent tab positions and mounted controls.
- Added a catalog-only month/year popover to both selectors and a floating Ask panel that retains drafts/requests/answers on close, clears on scope changes, and routes source links once after exit. Updated React to 19 for Konsta compatibility and migrated boolean inert contracts. Native button semantics, Safari focus return, focus boundaries, viewport containment and reduced motion were verified.
- Kept financial controllers outside monthly content, including the report:null loading/error transition found in full-browser QA. Month requests hide old monthly values while preserving open savings/payment drafts and exact uncertain retries. Explicit app updates and signout cannot interrupt tracked requests or unresolved financial outcomes; unsaved drafts prompt before discarding. The Update/× toast is session-dismissable per version, with the action retained in Settings.
- Verification: full suite **1,445 passed, 177 skipped** (environment-dependent contracts), one existing Kaleido deprecation; Pyright **0 errors/warnings**, frontend typecheck/build and diff check passed. Real production-build WebKit passed 320/390/1280px in both themes, all four charts/screens, scroll compaction/restoration, retained forms through month loading, failed/then-recovered refresh, six comparison selections and recovery, uncertain payment retry identity, Settings guard/Toast, Ask close/reopen and source navigation. Isolated real Konsta checks additionally cover 12 anchored popover layouts and Ask keyboard/reduced motion/480px viewport height. No live financial test records were created.
- Updated README, phone setup/acceptance guidance and architecture/workflow decisions. Bundled frontend fingerprint: `443750ab3f61e94bbc467d9b`. Implementation commit `70be82f` passed full GitHub Verification [34327707408](https://github.com/brianjames-dev/bookiebot/actions/runs/34327707408). The additional executed shell regression covers report:null loading, failed-month recovery and expiry cleanup; its focused wrapper also passes. Optional tall Ask sheet remains deferred as agreed.

## Completed 2026-09-08

- 2026-09-08 — Simplified zero-debt directions without hiding actual history or pending/error/sync recovery. Removed empty list disclosures and replaced the View expenses glyph swap and comparison +/x rotations with the existing vertical-stroke +/− transition. Added real React regressions for direction switches, settled history, pending confirmation, sync warnings and marker identity through repeated toggles. Eleven focused UI/motion checks, typecheck and build passed, plus 46 WebKit layout checks and 32 animation cycles across 320/390/1280px, both themes and reduced motion. Release evidence follows in the completion response. No financial logic or stored data changed. The user subsequently selected preview A; borderless editorial direction styling passed the lifecycle regression, typecheck/build and 72 WebKit layout/style checks plus eight bidirectional underline cycles. Release evidence follows in the completion response.

- 2026-09-08 — Implemented the approved confirmed-repayment ledger and compact both-direction phone UI. Payer names come from authenticated actors or explicit payer/account grammar, never a name inside an item; legacy mutation paths fail closed on missing settlement state or canonical linkage. Durable integer-cent allocations/events, household locking, exact request replay, version checks, pending confirmation, offsets and reversals feed verified source/payment/ledger projections. Migration preserves received history and validates every unpaid source before registration. README, phone guide, reimbursement operator guide, decisions and finance backlog updated. Final full suite: 1,617 passed with real PostgreSQL; Pyright/typecheck/build and Apps Script checks passed. Bank matching retains exactly one original gross candidate after partial/full repayment; projection read counts were reduced without skipping identity checks. Production migration completed with all seven allocations projected and independently verified; the reviewed payer/name correction is included. Main GitHub Verification passed, and the live frontend fingerprint matches the build. The disposable PostgreSQL test container was removed. No synthetic receipts, offsets or real money transfers were made. Live UI evidence is recorded in the completion response; device acceptance is checklist 109.
...
- 2026-09-08 — Removed repeated decorative dividers after Expense Highlights, Savings goals, Ask BookieBot and Phone notifications, plus savings rows/history/error notices and question answers. Scoped last-child overrides prevent the shared disclosure border from reappearing with production CSS order. Kept existing spacing, financial progress bars, control boundaries and archive-menu grouping. CSS-only; 10 existing UI/motion checks and frontend typecheck/build passed. Updated checklist 107; no workflow or data changes.

- 2026-09-08 — Implemented approved Savings goals simplification: removed the section introduction, compacted initial rows to exact fitted amounts/progress, moved expansion chevrons left and actions to a separate ⋯ menu. Overview retains deadline/target/remaining/progress; completed and archived goal dates stay discoverable, and Starting/Added summaries move to History. Coordinated one goal/view at a time and retained separate animated view exits. Existing ReportMenu gained optional label/disabled/close-on-select while keeping masthead behavior. Added real React lifecycle regressions for lazy history/paging/cancellation, menu/accordion state, exact cents, confirmed archive/restore/reversal, fresh versions, uncertain-write idempotent retries and conflict recovery. Focused 41 passed / 27 optional database skipped; full 1,116 passed / 61 optional database skipped; Pyright/typecheck/build clean. WebKit verified six viewport/theme layouts, eighteen edge layouts, keyboard/touch, unclipped active/archived menus, symmetric 240ms motion and all synthetic goal actions with no overflow/runtime errors. Fixed inherited bright hover fill and toned down progress tracks to match the approved design. No financial/storage behavior changed. Phone guide, presentation decision and checklist 106 updated.

- 2026-09-08 — Implemented approved reimbursement visual A: a 24px standard bar shows original gross as own share, actual partner receipts and hatched remaining debt, with high-contrast ivory payer/total text. Two aligned keys replace repeated label/value rows; due is keyed only on expansion. Exact-cent validation prevents misleading proportions and retains five labeled values for inconsistent/zero/legacy data. Full dates remain accessible, dynamic partner/payer names and fronted/differing budget metadata remain correct, and long names/amounts wrap safely. Expanded due sizing uses an independent flex basis to prevent a browser-detected ResizeObserver shrink loop; the hatch sits beside due without a detached gap. Existing overview, Totals, per-month accordion, source reads and financial behavior are unchanged. Added allocation/render/fallback regressions, phone guidance and a presentation decision. Focused 15 passed; full 1,116 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. WebKit passed six viewport/theme layouts, 22 edge states and 36 extra responsive edge cases, with exact proportions, minimum 5.45:1 ivory contrast, keyboard/rapid toggles, symmetric 240ms motion, no overflow/runtime errors and identical overview pixels. Checklist 105 covers phone acceptance.

- 2026-09-08 — Consolidated reimbursement date/location and payer/split/partner copy above four flush label/value amount rows. Repeated responsibility is omitted; differing budget labels and fronted semantics remain explicit. Month headings are 18px with restored current/selected-month pills. Each keyed month now owns one expanded allocation ID, so opening a sibling closes the prior row while other months and Totals remain independent. AnimatedDisclosure supports paired controlled props and preserves standalone functional toggling and retained/inert content. Focused reimbursement/motion 76 and full 1,116 tests passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build clean. WebKit passed six viewport/theme layouts and sixteen edge states, including intermediate 240ms motion, rapid and keyboard switches, long values, missing metadata, fronted copy and state preservation. All overview images match prior HEAD exactly. Phone guide, disclosure/copy decision and checklist 104 updated. No financial calculations, history, source reads, storage or sheet mutations changed.

- 2026-09-08 — Changed reimbursement month bands to serif section headings with an underline and replaced the information icon with a labeled Totals disclosure and rotating chevron. Removed repeated expense titles; long names expand in the original summary. Expanded details now form a receipt with labeled date/partner/location, prominent gross, paired shares, a separate received row, and split-method/responsibility fields. Currency values fit their columns; blank optional metadata is omitted. Scope, receipt and disclosure tests retain all values and cover one-title behavior plus whitespace fallbacks. Focused 69 and full 1,116 tests passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build clean. WebKit passed six viewport/theme layouts and fourteen edge states with no overflow/errors, full long titles and accessible controls. Overview image comparison matches prior HEAD exactly. Phone guide and existing presentation decision updated; checklist 103 covers acceptance. No backend, financial calculation, history, storage or sheet-write changes.

- 2026-09-08 — Refined shared reimbursements into one cent-proportional stacked outstanding bar with matching month colors and dollar legend. Monthly totals moved from row footers into a labeled, keyboard-accessible info disclosure beneath the selected month heading; date/partner/audit data stay in compact expense row details. Month headers now have distinct shading and accents; rows and info targets are 44px. Updated regression contracts for proportional composition, matching legend identity, retained financial scope and real animated/inert totals toggling. Focused 69 passed; full 1,116 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. WebKit passed 320/390/1280px in both themes, thirteen edge states and two exact user-amount fixtures, with no overflow/errors and 240ms symmetric motion. Updated phone guide, presentation decision and this checklist 102. No backend reads, receipts, storage, financial calculations or mutation flows changed.

- 2026-09-08 — Diagnosed live refresh/history failures as Google Sheets ReadTimeouts also affecting unrelated reminders. Connectivity recovered without a transport change, but the old partial-catalog warning remained until Retry. Added safe typed source_timeout responses and per-year diagnostics, separate browser-deadline/generic messages, and one bounded catalog recovery after a new successful report. Partial month options, old report/timestamp and view preferences remain; failed refreshes do not retry catalog reads. No polling, timeout increase, sheet writes or financial changes. Focused backend 77 and frontend 3 passed; full 1,116 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. Real FreshExpenseApp WebKit at 320/390px verified source/deadline failures, deferred catalog completion, single recovery/no loops, preserved Projected/metrics/timestamp, manual Retry and comparison recovery, with no overflow/errors. Phone guide and recovery decision updated; checklist 101 covers acceptance.

- 2026-09-08 — Combined reimbursement expenses and statement into one month-grouped list with inline Due/Received, compact row summaries, expandable complete audit fields, and current/selected-month markers from coverage.asOf. Removed redundant captions and progress-track styling; bars show exact remaining amounts against one dollar scale. Optional canonical received-history payload keeps settled carried expenses visible across report months with no extra reads. Allocation-ID dedup prevents duplicate rows; selected-month statement totals exclude old receipts, and future selected-month items do not enter open-only totals. Updated scope/date/receipt/disclosure regressions, phone guide and presentation decision. Focused backend/report 97 passed and two focused UI checks passed; full 1,099 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. Initial sandboxed full run could not bind local HTTP fixtures; rerun with local server permission passed. WebKit 26.4 verified 320/390/1280px in both themes plus twelve edge states, keyboard and symmetric 240ms motion, long labels/amounts, old received-only history, dedup, historical/current labels and incomplete coverage, with no overflow or errors. No settlement, storage schema, financial total or sheet mutation changes.

- 2026-09-08 — Simplified shared reimbursements with a fitted outstanding total, matching-scope expense-month bar chart, collapsed oldest-first partner ledger, and separate selected-expense-month statement/received history. Cent-based aggregation, a four-row cap with earlier-month grouping, explicit undated amounts and accessible exact labels preserve every balance. Incomplete coverage stays visible; no monthly receipts are mixed into all-month graph totals. Added runtime financial/scope/date/disclosure regressions. Focused 66 passed; full 1,096 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. Real WebKit at 320/390/1280px in both themes verified compact first view, keyboard controls, symmetric 240ms intermediate animation frames, long amounts/names, multiple months/partners, partial/settled/empty/legacy states and no overflow/errors. Phone guide and presentation decision updated; no reconciliation, settlement, receipt lifecycle or sheet changes.
...
- 2026-09-07 — Fixed theme dropdown taps lost to Safari null-target blur by dismissing on outside focusin/pointerdown instead. Regression covers blur-before-click, internal focus, outside focus/tap, Escape focus return and cleanup. Moved the Daily Spending button reset before its today modifier, restoring contrasting primary-foreground text and intended bold weight in both themes. Targeted 7 passed; full 1,096 passed / 61 optional PostgreSQL skipped, Pyright/typecheck/build clean. WebKit reproduced the prior failure and passed touch/keyboard/reload checks; Chromium passed repeated label/icon/state toggles, system-theme override and reload. WebKit computed today contrast: dark 8.11, light 6.77. No financial data or notification delivery changes.

- 2026-09-07 — Fixed notification Enable/save rejecting its own POST/DELETE requests. Global no-referrer plus explicit same-origin fetch made Origin null, triggering the screenshot 403 before persistence. Add per-request same-origin referrer policy to notifications and app-session reconnect/signout requests; keep global privacy, strict configured-origin guard, same-origin cookies and redirect rejection. Live unauthenticated probes confirm correct origin 401 versus null 403 with exact message. Real HTTP/SQLite regression covers rejected enable, retry with retained endpoint, durable settings reload, preference edits, off/re-enable, owner isolation and revocation; Postgres variant runs in CI. Frontend mutation contracts retain origin policy and pending-subscription cancellation. Full local 1,095 passed / 61 optional PostgreSQL skipped; Pyright/typecheck/build passed. Actual component in WebKit 26.5 reproduced old Origin:null 403; fixed concrete-origin POST/DELETE enabled On, retained changed preferences through reload and reused the endpoint after Off/re-enable. Chromium 148 fixed flow also passed; its old request did not reproduce the bug. Temporary browser fixtures had no external requests or page errors. No production settings were changed and no phone notification was sent.

- 2026-09-07 — Fixed repeated comparison months exhausting Sheets quota. Production read-only logs confirmed per-user read 429s and report 503s. New per-workbook metadata/value batching preserves full canonical reports; comparison loads only annual monthly grids/schedules and reuses successful inputs/catalogs for at most 60s. Actor/owner/persons/year keys, bounded caches/epochs, and refresh generation invalidation prevent identity crossover or stale repopulation. Older full-report handoffs also carry launch generations; rejected work leaves no orphaned epoch entries. Failed/partial reads remain retryable; quota responses identify Sheets with safe logs, Retry-After 60 and clear phone feedback. Actual offline gspread/requests traffic reproduced the old third-comparison quota threshold (20 initial, then 39/58/77 reads) and reduced refresh plus all 8 comparisons to 10 reads; after editing synthetic sources, refresh plus all 8 again totals 20 and verifies changed selected/baseline values. Full local 1,093 passed / 60 optional PostgreSQL skipped, Pyright 0 errors, frontend typecheck/build passed. No production financial writes or report reads were used for QA.

- 2026-09-07 — Fixed refresh/comparison recovery paths. Regressions first demonstrated duplicate full-month source reads causing comparison 503 after a successful refresh, and a browser abort that never settled leaving the comparison slot occupied. Comparison now waits during main refresh, clears failed entries once per refresh, and uses an independently bounded abort promise. Server comparisons can reuse a successful report for at most 10s; actor/owner/persons/month keys, capacity bounds and invalidation on any new direct read prevent stale fallback or identity crossover. Gspread transport gets 5s connect / 20s read timeouts before cache publication; threads retain their semaphore until they actually exit. Targeted 44 passed; full 1,051 passed / 60 optional PostgreSQL skipped; Pyright/typecheck/build clean. Phone preview recovered both banners without reopening and preserved Projected. No production financial reads or writes were used for reproduction.

- 2026-09-07 — Dashboard reliability/polish: comparison uses one retained request per displayed report, parallel bounded builds, previous-month default and configured-month dropdown; refresh/unmount invalidate results, and either current comparison side clips to elapsed days. Notification Enable preserves native Safari activation; preferences autosave, same-person reconnects reclaim retained endpoints, and PostgreSQL serializes endpoint claims. Modal scroll lock preserves normal page geometry, title receives initial focus, and Close is centered. Burn Rate shows all five metrics, category filters lead on phones, chart numbering/daily footer rule removed, owner/refresh repositioned. Full regression: 1,104 passed including real PostgreSQL, Pyright 0 errors, frontend typecheck/build and Apps Script checks passed. Browser: 320/390/1280px, both themes, 12 rapid comparison selections, historical-to-current fair dates, 16 Current/Projected plus category changes, exact modal chart rectangles/scroll/timestamp before and after, and keyboard focus return. No external phone sends or live financial mutations used.

- 2026-09-07 — Fixed intermittent Current/Projected blank page. Recharts briefly supplies previous sectors while synchronizing the new data; indexing that new array by an old sector index threw on removed categories and could target the wrong surviving category. Resolve each sector through its payload category key, disable departed slice interaction and keep current amounts/keyboard drilldowns intact. Existing chart animation remains enabled. New real React/Recharts regression fails on the prior source and passes the fix. Targeted checks 6 passed; full suite 1,022 passed / 57 optional PostgreSQL skipped; Pyright, frontend typecheck/build and diff checks passed. Browser reproduction and rapid mode/filter switches passed after the fix.
...
- 2026-09-07 — Dashboard pass 6: optional per-phone Web Push, durable subscription/keys/preferences and delivery claims, weekly check-ins and optional tomorrow-payment alerts, Pacific delivery hours, private amounts by default, explicit enable/test/off controls and retry UI. Session expiry/reset/signout stop eligibility; expired endpoints are removed only if still the sent subscription. Real SQLite/Postgres and mocked signed/encrypted transport tests pass; real React unmount/permission races, abort/timeout and retry contracts pass. No external test push was sent. Manual check on each installed iPhone app: enable/allow, explicitly Send a test, set preferences, then Turn off and confirm status.
...
- Added explicit Expected Income Amount semantics: biweekly estimates always use the configured per-paycheck amount; fixed monthly uses the configured monthly net target. Actual smaller/larger paychecks never change the expected amount. Blank/zero/invalid expectation disables estimates with an explanatory report label. Biweekly matching uses an inclusive +/-3-day date window, groups split deposits into one period, and checks adjacent months/years without moving actual income between receipt months. Undated/out-of-window deposits remain actual-only receipts.
- Updated live Apps Script to match the repository, including removal of its older automatic trailing-placeholder creation/formatting helper. New month settings reference the latest configured prior monthly grid, preserving explicit overrides and propagating later corrections. Annual Templates carry the previous year's plan; both annual seed workbooks were inspected read-only and have the supported B4:D4 layout. Daily runs leave existing monthly layouts alone.
- Executed the targeted four-tab migration successfully. Verified every prior value/formula remains equivalent after the intended three-row shift. Updated six September personal-budget action references and two September split-ledger references; historical rows and other actions were unchanged. Legacy delete snapshots are trimmed so undo cannot recreate the removed overlapping controls. New-grid income logging/delete/undo tests preserve B4:E5 without capturing settings as transaction-row styling.
- Verification: full suite `597 passed` (one existing Kaleido warning), Pyright `0 errors`, executed Node migration/rollover/reference tests, and clean `git diff --check`. All four layouts and dropdown choices were visually checked; final API metadata confirms only the new settings controls remain. Backups are ignored under `data/income-settings-v2/`. Existing Template `#REF!` and Hannah zero-income `#DIV/0!` outputs are unchanged.
...
- Kept the carousel shell and every slide flush with the viewport edges while adding separate responsive inner content gutters: `16px`-to-`56px` horizontally and `16px`-to-`32px` vertically. The shared shaded surface still enters and exits from the screen edge, but chart labels, plots, and controls no longer touch any edge. Category Mix, Burn Rate, Calendar, and Bills & Utilities use the same header/body/footer grid, with the graph occupying whatever middle space remains and the lower control region aligned across pages.
...
- Added Recology regressions covering May/August quarterly hits with omitted June/July points and requiring the valid hits to remain connected. A local browser fixture confirmed one May-to-August Recology line with dots only at May and August while monthly utility series remain continuous, with no browser warnings or errors.
- Verification: report suite `47 passed`; full suite `490 passed`; Pyright reported zero errors; frontend typecheck/build passed; `git diff --check` passed. Production verification is checklist item 77 below.
...
85. Final horizontal income settings and expected-paycheck workflow (supersedes settings checklists 83–84):
    - Live layout verified on both Template and September tabs: B5 source, C5 mode, D5 expected net amount, E5 biweekly anchor. Row 6 is empty; B7:D7 is the transaction header. Both modes default/preserve biweekly; enter an amount in D5 to enable estimates.
    - In a test workbook, set xAI / biweekly / 3775 / 7/2/2026. Log $3,100 on September 10 and $50 rewards: Current is $3,150; Projected is $6,925 with only September 24 estimated at $3,775. Test September 7/13 (matched), September 6/14 (unmatched), split deposits, and undated receipts.
    - Verify August 31 receipt fulfills September 2 and October 2 receipt fulfills September 30 without shifting actual dollars. Repeat across December/January. Changing the anchor changes current/future cadence, never earlier-month settings.
    - Select fixed monthly and enter 6000 once. With $3,100 matching salary and $50 rewards, Projected is $6,050 with a $2,900 month-end remainder. Changing the anchor has no effect. Clear D5 or choose off to stop estimates.
...
2026-09-09 standalone student loan: full local **1,698 passed / 213 optional PostgreSQL skipped**, existing Kaleido deprecation only; Pyright **0 errors**, frontend typecheck, Apps Script migration/rollover and diff checks clean. Tests cover authenticated intent/amount guards, exact unique source matching, insertion-safe writes and later undo/update, uncertain-write handling, malformed amounts, old nine-column schedules, duplicate subscriptions/reminders and Current/Projected/calendar/widget parity. Chrome verified live source formulas; activation remains approval-gated as documented above.

2026-09-09 widget collection v1.4: full local **1,538 passed / 213 optional PostgreSQL skipped** (existing Kaleido deprecation only); Pyright **0 errors**; frontend typecheck/build and diff check clean. Actual HTTP producer→distributed Scriptable consumer covers both people/modes, selected goals, read-only shared balances and isolated offline summaries. Executed native-stack/DrawContext browser approximation passes **160 size/theme/data cases**, including full timestamps/amounts and centered Medium layouts. Production-build WebKit Settings passed **320/390/1280px × dark/light**, including 44px controls, long names, copy/retry/cancellation and Back preserving pairing drafts. Frontend fingerprint **03822c4766f9ad9005f1b53f**. Native iPhone acceptance is checklist 118; GitHub/PostgreSQL CI and live deployment evidence belong in the release response.

2026-09-08 confirmed reimbursements: **1,617 passed**, including PostgreSQL and real React UI contracts (one existing Kaleido deprecation warning). Pyright **0 errors**; frontend typecheck/build, Apps Script and whitespace checks passed. WebKit covered 48 responsive theme/state combinations, 320/390/1280px layouts, 240ms disclosure transitions and synthetic receipt/report/confirmation/offset/reversal/retry lifecycles. Projection faults, concurrent commands, session-expiry replay, both owner directions and original gross bank matching have focused regressions. Production rollout is tracked separately; no synthetic financial writes belong in live data.

2026-09-08 compact Savings goals: focused savings UI/phone API/store/motion **41 passed, 27 optional PostgreSQL skipped**; full unit suite **1,116 passed, 61 optional PostgreSQL skipped** (existing Kaleido deprecation); Pyright **0 errors**; frontend typecheck/build clean. Actual WebKit component QA at 320/390/1280px in both themes covered collapsed/expanded/menu states, one-open rapid switches, 240ms intermediate enter/exit frames, menu keyboard/Escape/outside focus and active/archived clipping, synthetic create/contribute/edit/history pagination/reversal/archive/cancel/restore, exact amounts and unique request IDs. Eighteen edge layouts include tiny/large/80-character names, zero, complete, overfunded, archived and empty. No page errors, overflow or production financial writes. Final targeted checks cover preserved completed dates and subtle hover/track styling. iPhone touch-mode WebKit also passed both themes: row/menu taps, contribution/cancel, outside dismissal and fitting without errors or overflow.

2026-09-08 reimbursement visual A: focused reimbursement UI/carry-forward/motion **15 passed**; full unit suite **1,116 passed, 61 optional PostgreSQL tests skipped** (existing Kaleido deprecation only); Pyright **0 errors**; frontend typecheck/build and diff checks passed. Actual-component WebKit verified 320/390/1280px in both themes, 22 edge states and 36 extra edge/width/theme combinations. Verified exact-cent proportions, five-value invalid-data fallbacks, settled/unpaid/fronted/partial cases, large amounts/long names, readable ivory text, marker visibility/alignment, keyboard/rapid independent-month accordions and symmetric 240ms motion. No page/ResizeObserver errors or external requests. Outstanding overview screenshots match HEAD exactly. Final marker alignment received focused browser and regression rechecks.
...
```bash
PYTHONPATH=src venv/bin/python -m pytest unit_tests -q
# passed: 597 passed, 1 existing Kaleido deprecation warning
...
Live Apps Script `upgradeCurrentIncomeSettings` completed at 23:36 Pacific on 2026-09-05. Four-tab API readback confirms B4:E5 controls, blank buffer row 6, B7:D7 headers, equivalent shifted formulas/values, six shifted action rows, and two shifted split-ledger references. Browser checks confirm all grids are legible and the mode dropdown exposes the three intended values. A final API cleanup removed legacy empty-column validations/notes; the saved script includes that cleanup for subsequent migrations.
...
# passed: quarterly Recology rendered a connected May-August line with dots only at those months; monthly series remained continuous; no browser warnings/errors
...
92. Mode-switch fix: load the update on each phone with Update now (or fully close/reopen once). Alternate Current/Projected, particularly where Income left or category coverage appears/disappears; repeat in Category Mix All/Needs/Wants/Savings and while another graph is selected. The dashboard must stay visible, and tapping a remaining slice must show that category’s current-mode amount and entries.

93. Dashboard reliability/polish acceptance: choose Update now or close/reopen the Home Screen app. Compare spending starts at Last month; choose another month, switch repeatedly, collapse/reopen, and refresh. Confirm matching date ranges and clear missing-history states. Open/close Category Mix, Calendar, headline and Daily Spending dialogs: background stays stable, Close is centered and only keyboard Tab highlights it. At phone width all five Burn Rate metrics fit, category filters remain at the top, owner sits beside BookieBot and refresh beside month. For each phone, explicitly Enable notifications if desired, confirm On, change time/type/privacy, reopen and verify settings persist. Use Turn off to disable the last type; use Save preferences to retry a failed save. Optional Send a test is the only delivery acceptance action and must be chosen by the owner.

94. Refresh/comparison recovery: load Update now once. Open Compare spending, tap refresh while it is loading, switch comparison month and collapse/reopen. It should settle with matching days and retain Current/Projected. If the connection fails, restore it and tap Try again or refresh; comparison recovers without restarting the app. In a synthetic environment, hold fetch/body promises beyond timeout and confirm Retry becomes available; never use production financial mutations to create failures.

95. Repeated-month comparison acceptance: load Update now on each phone. Open Compare spending and select at least five distinct months (all available months if convenient), then tap Refresh with comparison open and repeat. Values should load without restarting and Current/Projected should persist. In a synthetic/test workbook, edit selected and baseline amounts, refresh, and confirm both comparison sides update. Main refresh always reads fresh data; comparison history can reuse source inputs for up to 60s between refreshes. If Sheets is temporarily busy from other activity, wait about a minute and use Try again; the message should identify Sheets rather than your connection.
...
99. Compact reimbursement acceptance: load Update now. Shared Reimbursements should show its outstanding balance and up to four proportional expense-month bars, with no expense list or monthly metric statement expanded initially. Bar amounts sum to the balance; Earlier expenses collects older months and Undated stays explicit when present. View expenses reveals partner totals and oldest-first entries; expand an entry to see every original split field. Open the month’s Expense statement for Received/Gross paid/Your share and received history. Try both themes and Current/Projected; the reimbursement balance remains independent of projection. Missing records retain a visible warning and must not suggest everything was repaid.

100. Unified reimbursement acceptance: choose Update now. Confirm the overview has no Across all months / By expense month captions; each bar is labeled with the amount still due for its expense month. Open View expenses: one list groups rows by original month, marks the current month, and shows Received or the remaining amount due directly on each row. Tap a row to see its full item, location/date, split method, responsibility and four split amounts; opening/closing stays smooth. Selected-month Gross paid / Your share / Received totals are inside this same list and exclude older receipts. Browse a historical report month and verify Selected month versus This month labels. In a test workbook only, settle an older expense and refresh: its row remains Received, its outstanding bar/balance decreases, and report Income/Spent/Left/Saved do not change. Check both themes at phone width; incomplete-coverage warnings remain visible.

101. Refresh/history timeout recovery: load Update now. Refresh should retain Current/Projected and the latest successful update time. If history is unavailable, a later successful report refresh should check it once and clear its warning when it recovers; explicit history Retry also remains available. In a synthetic test only, return source_timeout for report/catalog/comparison reads: the app identifies the source timeout, keeps the prior report and usable month options, and never loops on failed attempts. Hold a request beyond the browser deadline and confirm a distinct taking-longer message; restore the source and retry to recover without restarting. No simulated failures or test financial writes should be introduced into production.

102. Reimbursement visual refinement: load Update now, then open View expenses. One segmented bar should fill the entire outstanding total; for $11.99 August + $106.90 September, portions are approximately 10% and 90%, with matching legend colors and exact dollar labels. Month headings have a distinct background; each expense shows its name and Due/Received on one compact line. Tap a row for its date, partner and all split amounts. Tap the selected month’s info button: Gross paid / Your share / Received appear directly beneath that heading and collapse smoothly. Check narrow phone widths and both themes; old received expenses remain visible and incomplete-history warnings remain outside disclosures. No financial writes are needed.

103. Reimbursement receipt styling: load Update now and open View expenses. Month names should read as serif section headings with a thin rule, with no filled heading band. Tap Totals beside the selected month: its chevron rotates and monthly values open/close smoothly. Expand rent or another expense: its name appears only in the summary, with a long name becoming fully readable there. Date/With/optional Location are separated from the prominent Gross paid, paired Your share/Partner share and the Received row; Split method/Expense for are labeled below. Check received, partial and outstanding examples in both themes and at narrow phone width; all amounts stay readable, and the overview bar remains unchanged. No test financial writes are needed.

104. Compact reimbursement accordion: load Update now and open View expenses. Month headings should be smaller and This month should appear in a pill; Totals still opens the month summary. Expand rent: its date/location and Paid by / Split … with … appear together above four aligned label/value rows. Fronted expenses say Fronted for; a different budget owner remains explicit. Open another expense in the same month: the prior row closes while the new one opens smoothly. Open an expense in another month: it remains independent; tapping an open row closes it. Check keyboard, rapid taps, long names/amounts, both themes and 320px width. Overview proportions, every financial amount and monthly totals remain unchanged. No test financial writes are needed.

105. Reimbursement visual A: choose Update now, then Shared Reimbursements → View expenses. Collapsed rows retain the title and plain amount due/Received. Expand an expense: a slim bar shows ivory payer/gross text, blue own share, solid brown partner-paid share and hatched remaining debt; the hatch marker appears beside due in the header. Below it, Yours and the named partner’s paid amount align with matching keys. Verify date/split/location and any fronted/different-budget explanation remain accurate. Check partial, unpaid and received examples already present; opening another row in the same month closes the prior row smoothly. Test both themes and narrow width; monthly Totals and the outstanding overview remain unchanged. Invalid legacy allocations retain five labeled amounts. No test financial writes are needed.

106. Compact Savings goals: choose Update now. All goals should start collapsed with name, exact amount set aside, a thin progress bar, a left chevron and a separate ⋯. Expand one for date/progress/target/remaining; open another and confirm the previous one closes smoothly. Completed goals have a full bar without a checkmark beside their balance, and their optional date remains available. Use ⋯ to inspect History (starting balance plus contribution records), Edit, Record contribution, and Archive; cancellation must leave balances unchanged. Archived goals expose History/Restore through ⋯. New goals open above the list. Check long names/large amounts, both themes and narrow width, keyboard/outside menu dismissal, and no clipped last-row menus. Use a disposable test goal only for actual create/contribute/reverse/archive/restore acceptance; record no test allocations against real goals. Existing uncertain-save Retry this change/Check latest messages remain visible outside disclosures.

107. Lower-dashboard dividers: choose Update now, then scroll from Expense Highlights through Savings goals, Ask BookieBot and Phone notifications. Section and goal-row separator lines should be gone while savings progress bars stay visible. Expand a goal, inspect its History and options, then open Ask BookieBot and notifications; their layout, controls and animation should remain intact. Check both themes and narrow width. No financial changes or notification sends are needed.

108. Savings overview and dates: choose Update now. Savings should use the lighter chart/reimbursement background and show the total recorded across every goal, followed by Goals and the existing compact rows. Archived balances count in the headline and are noted; switching report month does not redefine this total. Open New goal/Edit goal and Record contribution on an actual iPhone: blank and filled date fields must fit like adjacent fields, retain visible focus, and open the native picker. Cancel without changing real goals. Check 320px through desktop, both themes, exact large/fractional totals, and the last goal options menu over the next section. Test actual contributions/reversals/archive/restore only with a disposable goal; totals must follow confirmed balances and remain unchanged on archive/restore. Mobile-emulated desktop WebKit cannot reproduce the iOS-only native padding renderer bug; phone acceptance verifies that platform-specific fix.

109. Confirmed reimbursements: choose Update now on each phone. Brian's Owed to you and Hannah's You owe must show the same obligations; names within item descriptions must not change the payer. Expand the corrected Gameday expense and check payer, gross, original income split and remaining debt. Inspect existing historical received rows without editing them. In isolated test data, record a partial receipt and verify original payer expense decreases by that amount, one debtor expense appears on its payment date, and original gross remains the only bank candidate. Report a sent payment from the debtor: balances stay unchanged until the recipient confirms; pending reports prevent competing receipts/offsets. Review equal allocations before confirming an offset, and verify its reversal restores the entire group. Repeat the same request after a simulated lost response, stale version, session expiry, moved source row and projection failure: no duplicate expenses or double settlement. Check both themes at 320/390/1280px, symmetric disclosures, date containment, one open expense per month and all refresh paths. Never create synthetic transfers or receipts in real financial data.

110. Reimbursement empty-state/motion polish: choose Update now. On You owe with zero balance, see “You don’t currently owe any money.” with no opposite-direction net summary or empty accordion. Owed to you has matching zero-debt wording. Real settled history must still expand if present; pending confirmation, synchronization and retry notices remain available. Toggle View expenses, Compare spending and its Details repeatedly: each + smoothly becomes − and reverses at the same 240ms pace as individual expense rows, with no x/glyph snap or layout movement. Verify 320/390px and desktop, both themes and reduced motion. Cancel any opened payment form; do not submit synthetic financial records. The selected editorial direction tabs show both balances in borderless columns with a short sage underline that slides to the selected direction. Check both choices, long amounts, both themes and reduced motion; no filled card background or outer rule should appear.


111. Four-screen phone app: load the new version, then check Overview, Spending, Shared and Savings on each phone in both themes. The header gear opens Settings and Back returns to the previous tab; theme and notification controls remain usable. On a long screen, scroll down/up to compact/restore the glass bar; returning to a tab restores its position. Check portrait/landscape, iPhone safe areas, actual software keyboard and Reduce Motion. Month and comparison popovers only enable available months; Current/Projected and all four charts remain correct through repeated selections, failed connections and Retry. Open an unsaved savings goal and reimbursement amount form, switch tabs and request another month; both drafts must remain when returning, including while that month is loading or has failed. Cancel these review-only drafts rather than submitting synthetic financial records. Ask from the header, close while a response is pending, reopen to see it, follow a source, and verify month/mode changes clear the interaction. For a later release, × dismisses only that update notice for this session; Settings still offers Update. Update/signout with drafts asks first, while a pending or uncertain real change blocks until resolved. Saved goals, notification settings, signed-in identity and reimbursement history survive the release.

112. Scriptable widget acceptance: update BookieBot, then use Settings → Widgets separately from Brian's and Hannah's signed-in accounts. Follow `docs/SCRIPTABLE_WIDGETS.md` to download/import the script, create a ten-minute one-use pairing and add a small or medium widget. Confirm each person's own name, avatar, Current/Projected mode and both figures against the same current-month Overview/Burn Rate values. Change one connection's mode and refresh its Scriptable preview; the other person's mode/access must remain unchanged. Check large amounts, dark/tinted Home Screen appearance and the timestamp/age on the actual phone. Tap the widget and verify the HTTPS `/app/expenses` destination opens the browser; reconnect that browser separately if needed, and verify the original BookieBot Home Screen icon still opens the installed web app. After that widget itself fetches online, check offline/stale behavior without advancing its source timestamp; iOS may delay refresh or purge its local cache. Remove a connection, then run it online to verify the numbers clear and Reconnect widget appears. Old rendered pixels cannot be erased remotely before iOS runs it again; remove the Home Screen widget to hide them immediately. Re-pair only if desired. No financial writes or actual transfers are needed for these checks.

113. Ask motion and persistent tools: choose Update in BookieBot, then scroll each screen and confirm the compact glass chat/Settings toolbar stays in the upper right without overlapping the brand at the top. Open and close Ask repeatedly using × and outside taps; it should slide evenly in both directions without a final jump or lingering green ring. Check both themes, portrait/landscape, actual iPhone keyboard and Reduce Motion. Type a draft, close/reopen to confirm it remains, then clear it without submitting. Keyboard Enter/Escape should return a visible focus indicator to chat. If following an existing answer source, its destination should open only after the panel closes. No financial writes or notification changes are needed.

114. Widget pairing recovery and Back: update BookieBot, open Settings → Widgets → Set up widget, and use both top/bottom Back buttons plus the gear; edited pairing fields and any existing unexpired setup code should survive, with the previous scroll position restored. Get a setup code should return below the fixed toolbar without creating a connection. Copy script should remain inside BookieBot; if clipboard permission is denied, select/copy the displayed source without leaving the guide. Public /app/widgets/help and /app/widgets/connect pages should both return via Back to Settings, with normal sign-in required if opened in a separate unsigned browser. On iPhone, pair by running the exact BookieBot script inside Scriptable and pasting a new private setup code; verify the owner/figures, then select that same script in Edit Widget with no parameter. To adopt v1.1, replace the existing script code without renaming. Verify unpaired/reconnect/no-data taps open that exact Scriptable script, while populated widgets still open the browser dashboard. Cancel or reject setup and confirm it stops at its prompt/error rather than showing another ambiguous preview. No financial writes or test notification sends are needed.

115. Widget v1.2 authorization recovery: after updating the web app, copy the current script from Settings → Widgets → Set up widget and replace the existing Scriptable source without renaming it; verify v1.2 in the first line. Run inside Scriptable. If pairing was lost, use one new private setup code from the same person’s account; do not reuse a consumed code. Verify the name and amounts, rerun the same script and choose Refresh & preview, then Refresh the BookieBot widget list: Paired becomes Connected with Last checked only after a successful read. Select that same script for both Small and Medium, Parameter blank and When Interacting left at Open App. Both should retain the connection; inspect Home Screen timestamp and tap routing on the actual iPhone. If secure storage reports an error, unlock/retry and retain the exact sanitized error rather than generating repeated connections. Remove obsolete connections only after identifying the working one. No financial writes or test notifications are required.

116. Selectable widget themes: replace the existing Scriptable source with v1.3 from Settings → Widgets → Set up widget, keeping its name and working pairing. Run Theme & preview, choose Editorial then Small; repeat for Two-tone and Medium. Verify the full amounts, name/mode, avatar and one source timestamp fit on the actual iPhone. A negative available amount must stay legible on the sage panel, and a stale preview must retain its source time and Stale label. Set two Home Screen widgets to the same script with Parameters editorial and two-tone; both retain the same account/mode/connection. Clear a Parameter to follow the saved default. Cancel either picker and confirm it leaves the preference unchanged. After a preview, iOS may redraw the Home Screen later. Verify normal widget taps still open the browser dashboard and recovery taps open Scriptable. Native Scriptable sizing/typography and iOS refresh timing cannot be certified by the browser geometry approximation. No new pairing, financial writes or notification sends are required for a working widget.

117. Compact Settings: accept the app update and open Settings on both phone sizes/themes. Appearance, Notifications, Widgets, Account and About should have separate lighter surfaces and open spacing, with no clipped controls. Widgets show compact name/status rows, direct Current/Projected selection, and one Setup & themes link. Open a connection’s ⋯ menu to see its full name/last check and Remove; cancel removal to retain access. Long names should stay within two lines on the row and be readable in options. Setup shows three short steps and initially closed Themes/Help; Copy script stays in place with a selectable fallback, and both Back buttons preserve edited pairing fields or an existing code. After Get a setup code returns to Widgets, verify ⋯ → Remove still opens confirmation on iPhone; outside tap, Escape and rapid toggling leave menus usable. Changes save through existing requests; no new Scriptable installation, pairing, financial writes or notification tests are required to review this design.


118. Widget collection: update BookieBot, then Settings → Widgets → Setup & widgets → Copy script. Replace the existing Scriptable source with v1.4, keeping its name and pairing. Customize each type/theme, select an active Savings goal if desired, and copy its parameter into Edit Widget for separate Small/Medium widgets. Compare Upcoming with current-month scheduled Calendar payments, Categories with the same-mode Needs/Wants remaining, Savings with its selected goal, and Shared with confirmed totals in both directions. Confirm the larger centered Medium Savings ring, balanced Shared columns, full timestamp and all values fit on the actual iPhone. Run Widget & preview across types and verify existing blank/theme-only Budget widgets stay Budget. Check source timestamps and offline Stale labels independently per widget; iOS controls redraw timing. Populated widgets still open the credential-free browser dashboard. Back controls must preserve Settings and pairing drafts. No re-pairing, financial writes or notification sends are needed.

119. Standalone loan: after approved J-column activation and actual/day confirmation, refresh Brian's report and verify the preserved recorded amount replaces the $59 expectation. In a disposable next-month fixture, check zero actual → Current $0 / Projected $59, day 12 calendar, canonical widget agreement, and unpaid Template status. Explicitly log a test payment only in that fixture and verify the loan row/undo, including inserted rows and prefix-name neighbors. Confirm a matching subscription prevents duplicate forecasts and standalone commands; Hannah's unconfigured standalone remains subscription-only. Review annual master row/configuration before January. Source audit and separate Template/shared-total findings are in `docs/STUDENT_LOAN_INTEGRATION.md`; no synthetic live transactions or notification sends.
