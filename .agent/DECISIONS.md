...
Extend existing owner-authorized grants only with bounded widget summaries through version-two typed routes. Budget retains its version-one response. Budget/category/upcoming share one minimal cached report snapshot; optional missing sections cannot break Budget. Category amounts use canonical mode budgets and post-cascade remaining. Upcoming selects at most two scheduled bill/subscription events from today to current-Pacific-month end, excludes income, and makes no unpaid/posted claim. Savings uses the owner's active goal catalog and one recorded goal balance; an explicit unavailable/foreign/archived identifier never selects a different goal. Shared sums canonical outstanding by direction, leaves pending sender payments outstanding and never retries sheet projections. Goals and Shared use separate bounded database-only reads, so unavailable Sheets cannot block them.

Revalidate the grant and current identity after awaited reads; no query may choose an owner, mode or date. Both source and device caches preserve source timestamps. Device snapshots are isolated by connection/type/goal and revocation removes all of that connection's cached summaries. Keep only minimal fields, never payment history or credentials. Existing HTTPS widget taps continue opening the browser dashboard without authentication in the URL; native iPhone rendering and refresh timing remain explicit device acceptance checks.
...
The bearer grants authorize only a minimal current-Pacific-month widget snapshot. They cannot authenticate phone cookies, read itemized data or authorize financial/settings mutations. Identity and source scope come from current actor mapping, with a second grant/scope check after awaited work. Reuse canonical `modeViews` Left (`incomeAfterExpenses`) and signed Burn Rate availability (`-totalDifference`); add an ISO source-generation timestamp instead of interpreting the human-formatted report date. Reuse the existing bounded/coalesced report worker. Widget-only memory retains at most 32 minimal snapshots for five minutes, both modes from the same source; an eight-second response deadline leaves an already-running shared build intact. Neither delayed work nor cache hits advance the source timestamp. Web refresh remains independent and bypasses this widget cache.

The delivered script binds requests to the configured HTTPS origin, blocks redirects and keeps the grant in Scriptable Keychain scoped to server plus script name. Cache only the minimum summary/avatar with FileManager.local, never iCloud; offline/failed refreshes show the original timestamp and stale state, while authorization failure clears credentials and cached amounts. Request refresh after 15 minutes, acknowledge iOS scheduling, and use a native relative date so age can advance between runs. An already displayed offline widget cannot be remotely erased; revocation takes effect at the server immediately and at the widget on its next run. HTTPS tap URLs contain no credentials and normally open the default browser, whose account/session is separate from the installed Home Screen web app. Actual iPhone import/render/tap acceptance remains explicit.
...
## 2026-08-11 - Model Fronted Expenses As 0/100 Responsibility Allocations

Decision: A shared expense paid by one person but wholly owned by the other uses the existing split lineage with canonical method `fronted`, payer share `$0`, and partner share equal to the immutable gross. Parser entities, Discord component values, action metadata, ledger rows, and report labels all use that same term; the superseded development-only `covered` value is not a stored-data alias. Natural wording such as “I covered this for Hannah” may still resolve to the Fronted action. The source action retains the payer/person and gross for bank reconciliation. The visible shared-expense row retains the gross amount but changes its Person to the responsible partner so that partner's budget/report owns the spending. `Shared Reimbursements` remains payer-owned and adds responsible owner, original person, and responsible person columns; counterpart queries may read that canonical ledger without mirroring rows. Reimbursement settlement remains ledger-only. Fronted mode is limited to shared expense rows until personal bill cells support explicit cross-workbook ownership.
...
## 2026-09-05 - Persist Explicit Income Plans Across Blank Months

Decision: Supersede the 2026-08-02 restriction that source/start configuration can inherit only from the immediately prior month. Resolve settings chronologically from earlier Budget tabs in the selected annual workbook and the configured/accessible previous annual workbook, then apply the selected month's values. Keep settings in visible adjacent label/value cells beside the income table; `Main Income Source` is preferred over the legacy biweekly source label. Blank means inherit, `off` explicitly disables projection, and changing the employer resets inherited mode/amount/start. Source matching accepts the normalized complete employer label or that label plus one of `income`, `paycheck`, `payroll`, `salary`, `wages`; no generic keyword, arbitrary single-income, or fuzzy substring fallback remains. Biweekly estimates retain the immediately-prior-month freshness boundary for actual paycheck amounts and require an observed or configured date; the hardcoded July 2 cadence is removed.

Add `Income Projection Mode` (`biweekly`, `fixed monthly`, `off`) and `Fixed Monthly Income` for a fixed monthly net salary. A nonblank fixed amount selects fixed mode unless that tab explicitly chooses another mode. Fixed projected income equals actual income plus max(0, configured salary minus actual matching salary). Additional income stays additional and actual salary above the target is never reduced. Represent any remaining salary as one labeled month-end planning estimate, not assumed biweekly deposits. Invalid explicit mode/amount disables the estimate with a report explanation. Current and completed-month totals remain actual-only; settings do not mutate transactions or sheet formulas. Both the web card and canonical agent report expose the resolved basis.
