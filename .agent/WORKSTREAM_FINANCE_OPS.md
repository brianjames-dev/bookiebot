...
- Removed the remaining carousel-side gutters so the slides meet both viewport edges. All four pages now use a common header/body/footer grid; graphs fill the variable middle row while the details-control row stays aligned at the bottom.
...
- Added focused source regressions and rebuilt the embedded report assets. Desktop and `390x844` checks confirmed stable `640px`/`540px` stage heights, exact viewport-width slides, aligned lower regions, cleaned headings, complete subscription details, page-scroll isolation, one marker per date, aggregate tooltips, accurate transfer-adjusted usage, and no horizontal overflow.
